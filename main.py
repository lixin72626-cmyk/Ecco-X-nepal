...
# --- WEB SERVER (Bot မအိပ်အောင် လုပ်ပေးမည့်အပိုင်း) ---
app = Flask('')
...
def run():
    # Cloud Platform တွေက ပေးတဲ့ Port ကို သုံးဖို့ဖြစ်ပါတယ်
...
async def start_spam(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❌ `/spam @username` လို့ ရိုက်ပေးပါ။")
        return
    ...
    await update.message.reply_text(f"🔥 {target_user} ဒီစောက်တောသားကို အရှင်သခင်အီကို အနိုင်ကျင့်ပြီ...")

    async def spam_loop():
        try:
            while True:
                for msg in messages:
                    await context.bot.send_message(chat_id=chat_id, text=f"{target_user} {msg}")
...
if __name__ == "__main__":
    # Web Server ကို နောက်ကွယ်မှာ အရင်ပတ်ထားမယ်
    keep_alive()
